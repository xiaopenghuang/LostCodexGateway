...
隧道出口、服务器直连、现有代理三方对照：
```
tunnel egress:  223.85.246.143   ← 经隧道 = 服务器出口 ✅
server direct:  223.85.246.143   ← 服务器自身直连出口（一致）
existing proxy: 206.251.53.87    ← 用户现有代理出口（不同，证明未混淆）
```
...
| 普通未经过网关的应用不因此改变原有出口 | ✅ | 全程未改系统代理/路由/环境变量；对照出口保持 206.251.53.87 不变 |
...
```
[2. 尝试连接] -> {"ok":true,"v":"连接成功：出口 IP 210.41.243.3"}
[5. 轮询快照] state: EGRESS_VERIFIED  egress_ip: 210.41.243.3
    ...
    [OK] 隧道出口 IP -> 210.41.243.3                                    ← 回退端点成功
[6. 断开] -> 已断开  状态: READY
[7. 导出诊断报告] -> %APPDATA%\LostCodexGateway\diagnostics_*.txt
```

关键点：出口 IP 210.41.243.3 与容器内服务器直连出口一致；ipify 在服务器侧不可达时自动回退第二个端点（ip.3322.net），证明多端点容错有效；直连对照失败被如实标记而非隐藏。
...
```
[1. 配置并连接] 连接成功：出口 IP 210.41.243.3
...
本地出口: 210.41.243.3 | 网关出口: 210.41.243.3 (IPv4)
