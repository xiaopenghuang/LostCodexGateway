...
| Windows OpenSSH | `C:\Windows\System32\OpenSSH\ssh.exe` OpenSSH_for_Windows **9.5p1** (LibreSSL 3.8.2)；`ssh-keygen.exe` 同目录存在 | 可用；注意 PATH 第一位是 Git 自带 `G:\Git\Git\usr\bin\ssh.exe` (10.5p1)，应用需**显式优先使用 System32 版本**，否则行为不一致 |
...
| Clash Verge Rev | **2.5.2**，安装在 `G:\Clash Verge\`，`clash-verge.exe` + `verge-mihomo.exe` 均在运行 | 版本比文档写作时新，需按 2.x 能力适配（只读探测 + 增量写入） |
...
| 经 127.0.0.1:2080 出口 | 正常，出口 IP `206.251.53.87` | 用户现有代理工作正常 |
| 其他代理进程 | `FlClashHelperService.exe`（PID 6048，服务态）存在但**无 FlClash 数据目录**，疑似残留服务 | 不影响；不触碰 |
| Codex CLI | **0.154.0**，npm 全局安装：`G:\VSCODE\nodejs\node_global\codex.cmd`（node v22.17.1 @ `G:\VSCODE\nodejs`） | Native Windows CLI，非 WSL2 |
...
| VS Code | 已安装（`G:\VSCODE\Microsoft VS Code\bin\code.cmd`）；`.vscode/extensions` 中**未见 codex 插件目录** | IDE 场景待装插件后实测，v1 标记「未验证/未覆盖」 |
| WSL2 | 启用，`Ubuntu-24.04` Running（WSL2 内核），另有 `docker-desktop` | WSL2 内未发现 codex；v1 对 WSL2 内 Codex 标记「不支持/需单独诊断」 |
| 开发工具链 | Rust 1.98.1 + cargo（已装用户级）；MSVC 14.50（VS2026 Build Tools @ G:\MVS\BuildTools，链接验证通过）；node v22.17.1 + npm（@ G:\VSCODE\nodejs）；无 pnpm；WebView2 Runtime 153.0.4234.32 | 满足 Tauri 2 构建要求；NSIS 打包用 Tauri 自带 bundler |
...
| CLI 专用启动器（进程级代理） | ✅ 可实现 | PowerShell 子进程注入环境变量；`G:\VSCODE\nodejs\node_global\codex.cmd` 可定位 |
