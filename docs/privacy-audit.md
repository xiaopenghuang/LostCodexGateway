...
| # | 问题 | 位置 | 处理 |
|---|---|---|---|
| 1 | **真实 VPS SSH 端点**：`154.40.48.25:64824`，且紧邻真实主机公钥与指纹 | `src-tauri/src/ssh.rs`（2 处） | 端点描述改为「实测环境」；主机公钥/指纹替换为合成测试向量（见下） |
| 2 | **三个真实出口 IP**：`210.41.243.3`（本机/网关出口）、`223.85.246.143`（服务器直连出口）、`206.251.53.87`（现有代理出口） | `docs/acceptance-report.md`（13 处）、`docs/risks-and-unimplemented.md`（1 处）、`docs/m0-environment-report.md`（1 处） | 替换为 RFC 5737 文档段，并在文档开头声明「已脱敏、一对一映射」 |
| 3 | **整机环境快照**：真实用户名 `C:\Users\1`、盘符布局、已装软件及版本、运行中进程 PID、残留服务 | `docs/m0-environment-report.json` | 移出仓库（保留在本地 `.workbuddy-ai/`），并加入 `.gitignore` |
...
1. **一对一映射**：`210.41.243.3 → 203.0.113.47`、`223.85.246.143 → 198.51.100.22`、
   `206.251.53.87 → 198.51.100.88`。因此验收记录中「本地出口与服务器出口**相同**」
   「对照出口**不同**」这类**相对关系**依然成立，被验证的是判定逻辑而非具体数值。
...
`privacy-scan.py` 目前对 `D:\Clash Verge`、`E:\a b` 这类**示意路径**报 `low`。
这些字符串出现在以下两类位置，均已人工确认无泄露：
